import 'package:flutter/material.dart';

///
/// This class should contains all the colors that will be used
/// in the app. If the colors is not here, add it.
class ColorsConst {
  // General Colors
  static const white = Colors.white;
  static const black = Colors.black;
  static const transparent = Colors.transparent;

  // Colors for fonts
  static const darkGrey = Color(0xff1F262D);
  static const negativeRed = Color(0xffEB5757);
  static const infoBlue = Color(0xff4C71F2);
  static const warningYellow = Color(0xffF2C94C);
  static const positiveGreen = Color(0xff40c97b);

  // UI Colors
  // static const primaryPurple = Color(0xff685BFB);
  static const primaryPurple = Color(0xffFE9B00);
  static const lightPurple = Color(0xffFE9B00);
  // static const lightPurple = Color(0xffE2E0FF);
  // static const primaryBlack = Color(0xff333333);
  static const primaryBlack = Color(0xff212E3E);
  static const grey = Color(0xff828282);
  static const beige = Color(0xffFFE4C9);
  static const veryLightGrey = Color(0xFFE5E5E5);
  static const lightGrey = Color(0xffD9D9D9);
  static const lightGreyPrimary = Color.fromARGB(255, 242, 240, 240);
  static const disableGrey = Color(0xffE2E7EC);
  static const orange = Color(0xffFE9B00);
  static const lightOrange = Color(0xffFFD8B4);
  static const darkBlue = Color(0xff212E3E);

}
